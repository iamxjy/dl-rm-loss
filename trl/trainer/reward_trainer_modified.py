...
class RewardTrainer(BaseTrainer):
    ...
    def compute_loss(
        ...
        loss_key = self.loss_type

        if loss_key not in self._loss_fns:
            raise ValueError(
                f"Unsupported loss: {loss_key}."
            )

        loss = self._loss_fns[loss_key](rewards_chosen, rewards_rejected, inputs)
    ...
    def _compute_bradley_terry_loss(
        self,
